...
import com.nwkyom.youcode.config.KafkaProperties;
...
class YouCodeKafkaResourceIT {
    ...
    void setup() {
        ...
        YouCodeKafkaResource kafkaResource = new YouCodeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/you-code-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/you-code-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
